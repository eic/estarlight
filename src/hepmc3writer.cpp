...
int hepMC3Writer::writeEvent(const eXEvent &event, int eventnumber)
{
  ...
  //hepmc3_beam_vertex_to_write->add_particle_out( hepmc3_electronBeam_particle );
  //hepmc3_beam_vertex_to_write->add_particle_out( hepmc3_targetBeam_particle );
  ...
  //hepmc3_vertex_to_write->add_particle_in( hepmc3_electronBeam_particle );
  //hepmc3_vertex_to_write->add_particle_in( hepmc3_targetBeam_particle );

  
  /** Get starlight particle vector **/
  const std::vector<starlightParticle> * particle_vector = event.getParticles();
/*
  lorentzVector beamElectron_lorentzVec = (*event.getSources())[0];
  HepMC3::GenParticlePtr hepmc3_beamelectron = std::make_shared<HepMC3::GenParticle>( hepmc3_electronBeam_four_vector_, 11, 4 ); 
  hepmc3_vertex_to_write->add_particle_in( hepmc3_beamelectron );
*/
